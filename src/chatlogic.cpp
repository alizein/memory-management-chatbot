...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // azein: (T3) Change raw pointer into smart pointer
                        ...
                        {
                            // azein: (T3) change raw pointer creation into smart pointer
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            // azein: (T3) assign address of object owned by smart pointer as a raw pointer to rootnode
            if (rootNode == nullptr)
            {
